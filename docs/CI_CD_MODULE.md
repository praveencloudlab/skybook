...
- Post-merge: the matrix's first real execution (the merge commit's `main` run) — all 8 jobs ran in parallel and pushed; `ghcr.io/praveencloudlab/skybook-flight-service:latest` pulled back anonymously and smoke-tested against the local compose Postgres, `/actuator/health` → `{"status":"UP"}`, identical to the locally-built image.

**SonarCloud setup, completed post-merge:** the user created the `praveencloudlab` SonarCloud organization (GitHub login), generated a token, and added it as the `SONAR_TOKEN` repository secret. No manual UI project import was done — deliberately: the first CI-based `mvn sonar:sonar` run auto-provisions the project under the pom's declared key (`praveencloudlab_skybook`), which also sidesteps the Automatic-Analysis-vs-CI-analysis conflict a UI import can switch on by default. `sonar.organization`/`sonar.projectKey`/`sonar.host.url` live in the root pom's properties alongside the JaCoCo report paths. The commit adding this paragraph is itself the scan step's activation run.
