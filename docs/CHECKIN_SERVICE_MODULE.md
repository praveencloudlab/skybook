...
| JPA — Testcontainers PostgreSQL | Every §3.1.1 DB-enforced invariant: one `CheckIn` per `bookingPassengerId`, unique `boardingPassNumber`/token/baggage tag, append-only history, `@PrePersist` defaults, version increment, finder/sweep queries. | ✅ Done — 13 tests |
...
- **`CheckInRepository.findDistinctFlightIdByDepartureTimeBefore` didn't do what its name implied**, found only once Docker became reachable and the JPA tests ran against a real Postgres container. Spring Data does not treat the property token between `findDistinct` and `By` as a projection selector the way the name suggests — it silently derived a full-entity `SELECT DISTINCT c FROM CheckIn c ...` query instead of a scalar `flightId` projection. That derivation is syntactically valid, so it passed Spring Data's startup validation and compiled fine; it only failed at result-binding time against the `List<Long>` return type, and only when actually invoked with real data. Fixed with an explicit `@Query`. No amount of Mockito-based service testing could have caught this — the repository method itself was never exercised against a real query planner until the JPA layer existed.
...
| JPA — Testcontainers (13) | `CheckInJpaTest`, `CheckInChildEntitiesJpaTest` | Real Postgres. Found and fixed the `findDistinctFlightIdByDepartureTimeBefore` bug above. |
| Integration (3) | `CheckInBookingEventKafkaIntegrationTest` | Real Postgres + real Kafka; the consumer-in/producer-out loop end to end, deliberately scoped (see above). |
| Concurrency (2) | `CheckInConcurrencyTest` | Real Postgres + real threads; the create-race actually produced a live duplicate-key violation in this run, not just a theoretical one. |

Every layer in the design doc's §17 testing plan now has at least a first pass. Not yet run: JaCoCo/SonarQube/OpenAPI/Postman deliverables for this module.

---

*Implemented per this doc's build order (§16, steps 1-13) and tested end to end through every layer of §17's testing plan (§18) on `feature/checkin-management`. Mirrors `PAYMENT_SERVICE_MODULE.md`'s structure and rigor throughout.*
