...
**5. Kafka consumer groups were silently non-functional - caught by user review, not by this doc's own first verification pass.** The single-node broker was left at Kafka's defaults, `offsets.topic.replication.factor=3` and `transaction.state.log.replication.factor=3`, which a 1-broker cluster can never satisfy - so `__consumer_offsets` never actually got created, and every consumer group's offset commits silently failed forever. The broker itself stayed "healthy" throughout (the `kafka-broker-api-versions.sh` healthcheck only proves the broker responds to API requests, not that topic creation or consumer-group coordination actually works), and application logs showed a normal-looking "Subscribed to topic(s)" line, so this was easy to miss - the first verification pass here did exactly that, taking "subscribed" in a log line as sufficient proof and moving on without checking `kafka-consumer-groups.sh --list` actually returned anything. It didn't - the list came back empty, which should have been the tell. Fixed by setting `KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR=1`, `KAFKA_TRANSACTION_STATE_LOG_REPLICATION_FACTOR=1`, and `KAFKA_TRANSACTION_STATE_LOG_MIN_ISR=1` on the `kafka` service - the standard, well-known requirement for any single-broker Kafka cluster. Re-verified properly this time: produced a real message onto `skybook.booking.events` via `kafka-console-producer.sh`, then `kafka-consumer-groups.sh --describe --group payment-service` showed `LOG-END-OFFSET=1`, `CURRENT-OFFSET=1`, `LAG=0` - an actual committed offset, not just a subscription log line. All four consumer groups (`payment-service`, `booking-service`, `checkin-service`, `notification-service`) now appear in `--list`.
...
- Kafka: all four topics exist, all four consumer groups are registered, and a manually-produced message's offset was actually committed by a real consumer (`kafka-consumer-groups.sh --describe`) - not just "a log line said subscribed," which is what the first pass incorrectly treated as sufficient (see finding #5).
